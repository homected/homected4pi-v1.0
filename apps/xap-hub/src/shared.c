...
short int hubSharedMemSetup(void)
{
	...
	if((hubConfig = (xAP_HUB_CONFIG*)shmat(shmHubConfig, NULL, 0)) < 0)
		return 0;
...
{
	shmdt(hubConfig);							// Detach the variable from the shared memory area
	shmctl(shmHubConfig, IPC_RMID, NULL);		// Destroy the shared memory area
}
