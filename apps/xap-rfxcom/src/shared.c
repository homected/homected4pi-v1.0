...
short int rfxcomSharedMemSetup(void)
{
	...
	if((shmRfxcomConfig = shmget(SHMKEY_RFXCOMCFG, sizeof(t_RFXCOM_CONFIG), IPC_CREAT | 0666)) < 0)
		return 0;

	// Attach the shared variable to the shared memory area
	if((rfxcomConfig = shmat(shmRfxcomConfig, NULL, 0)) < 0)
		return 0;
...
{
	shmdt(rfxcomConfig);						// Detach the variable from the shared memory area
	shmctl(shmRfxcomConfig, IPC_RMID, NULL);	// Destroy the shared memory area
}
